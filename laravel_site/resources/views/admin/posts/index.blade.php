		...
		<div>
			<form action="" name="adminForm" id="form-admin" method="POST">
				...
				<div class="px-5 pt-5r">
					<h4 class="ms-5 fw-bold text-red-primary mb-3">Category: {{ $category->name }}</h4>
						...
						<div class="mt-4 table-responsive">
									...
									<tbody>
										@foreach ($posts as $i => $post)
											<tr>
												...
												<td>{{ $post->title }}</td>
												<td>
													<span class="fw-medium">{{ !empty($categories->find($post->category_id)) ? $categories->find($post->category_id)->name : '' }}</span>
												</td>
												<td>{{ $post->created_at->format('d-m-Y H:i:s') }}</td>
												<td class="post-status {{ $post->active == 1 ? 'active' : '' }}">{{ $post->active == 1 ? 'Activated' : 'Deactivated' }}</td>
												<td class="">
													<div class="d-flex align-items-center">
														<a href="{{ url('/admin/category/'.$category->id.'/posts/edit/'.$post->id) }}" class="me-3"><img src="{{ asset('assets/images/old_img/icon-eye.svg') }}" alt=""></a>
														<a role="button" onclick="return confirmBox('Delete items', 'Are you sure you want to delete the selected items?', 'itemTask', ['{{ $post->id }}', 'delete'])"><img src="{{ asset('assets/images/old_img/icon-trash.svg') }}" alt=""></a>
													</div>
												</td>
											</tr>
										@endforeach
							...
							<div class="d-flex justify-content-between mt-4 flex-wrap">
								...
								<div class="d-flex gap-3">
									...
									<a href="{{ url('/admin/category/'.$category->id.'/posts/edit') }}" class="btn btn-red-400 btn-add-post">
										Add new
									</a>
								</div>
							</div>
